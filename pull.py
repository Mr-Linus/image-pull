...
def run(host, image_name):
    with grpc.insecure_channel(host + ':50051') as channel:
        ...
        response = stub.CheckImageExists(
            image_pull_pb2.ImageName(
                Image=image_name))
        if response.stats == 0:
            ...
            print("The image does not exist in the repository, \n" +
                  ...
                  image_name)
            response = stub.PullImage(
                image_pull_pb2.ImageName(
                    Image=image_name))
            if response.stats == 0:
                print(
                    "Sync Finished,Mirror image is:" +
                    response.ImageMirrorName)
                client = docker.client.from_env()
                print("Pulling images...")
                image_object = client.images.pull(response.ImageMirrorName)
                print("Changing the image's tag...")
...
def main():
    parser = optparse.OptionParser(
        'usage: pull.py -i <imagename> e.g: quay.io/coreos/flannel:v0.11.0 [-f <imagename-file>] | [-s <mirror server>]'
    )
    ...
    if (options.image is None) and (options.filename is not None):
        try:
            ...
            print(
                "Read the file:" +
                options.filename +
                " Error! or Docker is not running!")
